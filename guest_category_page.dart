           ...
           ///////////////
            actions: [
              ...
              Padding(
                ...
                child: Center(child: Text(formatYen(cart.total))),
              ),
             ////////////////////////////////////////
              IconButton(
...
class _PromoSplitPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        Expanded(child: _PromoPanel(promos: top)),
        const SizedBox(height: 12),
        Expanded(child: _PromoPanel(promos: bottom)),
      ],
    );
  }
}
          ...
          PageView.builder(
            ...
            itemBuilder: (context, i) {
              ...
              return _PromoTile(
                promo: p,
                onTap: () {
                  if (p.linkType == 'brand' && p.category != null) {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                  ...
                  if (p.linkType == 'category' && p.category != null) {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (_) =>
                            BrandListPage(category: p.category!),
                      ),
                    );
                    return;
                  }

                  if (p.linkType == 'brand' && p.category != null && p.brand != null) {
                    final menu = context.read<MenuData>();
                    Map<String, dynamic>? selected;
                    for (final item in menu.items) {
                      if (item['category'] == p.category && item['name'] == p.brand) {
                        selected = item;
                        break;
                      }
                    }
                    ...
                    if (selected == null) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text('選択されたブランドが見つかりません')),
                      );
                      return;
                    }
                    ...
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (_) => _PromoBrandVariantsPage(
                          brandName: (selected!['name'] ?? '').toString(),
                          variants: List<Map<String, dynamic>>.from(
                            selected!['variants'] ?? const [],
                          ),
                        ),
                      ),
                    );
...
class _PromoBrandVariantsPage extends StatelessWidget {
  final String brandName;
  final List<Map<String, dynamic>> variants;

  const _PromoBrandVariantsPage({
