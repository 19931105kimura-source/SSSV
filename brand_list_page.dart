...
class _BrandListPageState extends State<BrandListPage> {
  ...
  Future<void> _showPrinterSettingsDialog(BuildContext context) async {
    ...
    try {
      ...
      final kitchenHostCtrl = TextEditingController(
        text: settings['kitchen']?['host']?.toString() ?? '',
      );
      final kitchenPortCtrl = TextEditingController(
        text: (settings['kitchen']?['port'] ?? 9100).toString(),
      );
      final registerHostCtrl = TextEditingController(
        text: settings['register']?['host']?.toString() ?? '',
      );
      final registerPortCtrl = TextEditingController(
        text: (settings['register']?['port'] ?? 9100).toString(),
      );
      final receiptHostCtrl = TextEditingController(
        text: settings['receipt']?['host']?.toString() ?? '',
      );
      final receiptPortCtrl = TextEditingController(
        text: (settings['receipt']?['port'] ?? 9100).toString(),
      );
      ...
      await showDialog(
        ...
        builder: (_) => AlertDialog(
          title: const Text('プリンターIP設定'),
          content: SingleChildScrollView(
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                _buildPrinterAddressFields('厨房', kitchenHostCtrl, kitchenPortCtrl),
                const SizedBox(height: 12),
                _buildPrinterAddressFields('レジ', registerHostCtrl, registerPortCtrl),
                const SizedBox(height: 12),
                _buildPrinterAddressFields('レシート', receiptHostCtrl, receiptPortCtrl),
              ],
            ),
          ),
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                final payload = {
                  'kitchen': {
                    'host': kitchenHostCtrl.text.trim(),
                    'port': int.tryParse(kitchenPortCtrl.text.trim()) ?? 9100,
                  },
                  'register': {
                    'host': registerHostCtrl.text.trim(),
                    'port': int.tryParse(registerPortCtrl.text.trim()) ?? 9100,
                  },
                  'receipt': {
                    'host': receiptHostCtrl.text.trim(),
                    'port': int.tryParse(receiptPortCtrl.text.trim()) ?? 9100,
                  },
                };
                ...
                try {
                  ...
                  if (!mounted) return;
                  scaffoldMessenger.showSnackBar(
                    const SnackBar(content: Text('プリンター設定の保存に失敗しました')),
                  );
                }
              },
              child: const Text('保存'),
  ...
  Widget _buildPrinterAddressFields(
    String label,
            ...
            actions: [
              ...
              if (isOwner)
                IconButton(
                  ...
                  tooltip: 'プリンターIP設定',
                  onPressed: () => _showPrinterSettingsDialog(context),
                ),
            ],
          ),
          body: Row(
