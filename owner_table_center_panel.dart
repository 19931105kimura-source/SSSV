...
class _OwnerTableCenterPanelState extends State<OwnerTableCenterPanel> {
             ...
             TextButton(
                onPressed: orderData != null
                    ? () async {
                        ...
                        if (!ok) {
                          if (!mounted) return;
                          await _showOrderDeleteSyncErrorDialog(context);
                          return;
                        }
  ...
  Future<void> _showOrderDeleteSyncErrorDialog(BuildContext context) {
    return showDialog<void>(
      context: context,
      builder: (_) => AlertDialog(
        title: const Text('注文削除'),
        content: const Text('注文削除の同期に失敗しました。再試行してください。'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
            child: const Text('OK'),
