...
final rtOrderIds =
    (rtState.ordersByTable[widget.table] as List<dynamic>?) ?? [];

final rtItems = <dynamic>[];

for (final orderId in rtOrderIds) {
  final items =
      (rtState.orderItems[orderId] as List<dynamic>?) ?? [];
  rtItems.addAll(items);
}
    ...
    final billing = orderData == null
        ? null
        : BillingCalculator.calculateFromLines(orderData.lines);
  ...
  Container(
    ...
    : () async {
        try {
          await http.post(
            Uri.parse(
              '$_serverBaseUrl/api/rt/tables/${widget.table}/start',
            ),
          );
        } catch (e) {
  ...
  onPressed: isActive
      ? () async {
          try {
            await http.post(
              Uri.parse(
                '$_serverBaseUrl/api/rt/tables/${widget.table}/end',
              ),
            );
            Navigator.pop(context);
          } catch (e) {
  ...
  child: const Text('終了'),
              ...
              TextButton(
                onPressed: orderData != null
                    ? () async {
                        await orderState.removeOrder(orderData.id);
  ...
  Future<void> _printReceipt(BuildContext context, String tableId) async {
    ...
    try {
      final res = await http.post(
        Uri.parse('http://SERVER_IP:3000/api/print/receipt'),
...
class _TimeDialogState extends State<_TimeDialog> {
  ...
  DateTime? _parseStartTime(String v) {
  ...
  String _formatEndTime(String? startTime, int totalSeconds) {
    final st = startTime ?? '';
    final start = _parseStartTime(st);
  ...
  startTime: _startTimeCtrl.text,
    ...
    Widget autoBtn() => ElevatedButton(
          ...
          onPressed: () async {
            ...
            // autoExtend ON の場合は表示なし（あなたの要望通り）
 ...
 Widget subBtn(int m) => OutlinedButton(
  ...
  child: Text('-${m}分'),
    ...
    Future<void> startPressed() async {
      ...
      // 初回スタート：もし未設定なら 0→にしてから始める
      // 「押して組み合わせ」前提なので、0のままは開始しない
      ...
      content: Column(
        ...
        children: [
          ...
          if (!autoExtend) ...[
            ...
            Text(
              '終了：${_formatEndTime(_startTimeCtrl.text.isEmpty ? startTime : _startTimeCtrl.text, totalSeconds)}',
