...
Order? _buildRealtimeOrder(String table) {
 ...
 return Order(
  ...
  Future<void> _deleteRealtimeLinesOnServer(String table, List<OrderLine> lines) async {
    final targets = lines
    ...
    for (final lineId in targets) {
      try {
        ...
        // RT 削除はベストエフォート。後続の sync-table で整合を取り直す。
      }
    }
  }
...
if (order == null) {
 ...
 Future<bool> removeOrder(String orderId) async {
    ...
    try {
      if (isRealtimeOrder) {
        await _deleteRealtimeLinesOnServer(targetOrder.table, targetOrder.lines);
      }
