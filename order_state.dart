...
class OrderLine {
 ...
 Map<String, dynamic> toServerItem() {
  return {
    'name': label,          // ★ Node 側で使う表示名
    'label': label,         // 互換用（残してOK）
    'price': price,
    'qty': qty,
...
Order? _buildRealtimeOrder(String table) {
  ...
  for (final it in rawItems) {
    ...
    final key = '$name|$price'; // まずは名前単位で合算

    if (!aggregated.containsKey(key)) {
      aggregated[key] = OrderLine(
      ...
      aggregated[key] = cur.copyWith(qty: cur.qty + (qty <= 0 ? 1 : qty));
    }
  }

  final lines = aggregated.values.toList();

  if (lines.isEmpty) return null;

  // 表示専用の Order を作る（id は仮）
  return Order(
    id: 'rt_$table',
    table: table,
    createdAt: DateTime.now(),
...
if (order == null) {
 ...
 Future<void> _startTableOnServer(String table) async {
  ...
  if (qty <= 0) return;
    ...
    if (idx >= 0) {
      final cur = order.lines[idx];
      order.lines[idx] = cur.copyWith(qty: cur.qty + qty);
    } else {
      order.lines.add(
        OrderLine(
          category: category,
