...
function printableReceiptItemName(item) {
  ...
  // セット系で「通常」「案内所」はブランド名を印字しない
  // 例: 「通常 60分」「案内所 60分」ではなく「60分」のみ表示
  const hiddenBrandNames = new Set(["通常", "案内所"]);
  if (labelName && hiddenBrandNames.has(brandName)) {
    return labelName;
  }

  if (brandName && labelName) {
    const shortBrandName = shortenReceiptDrinkName(brandName);
    return `${shortBrandName}）${labelName}`;
  }


  if (brandName) {
    return shortenReceiptDrinkName(brandName);
  }
...
async function printOrderSlip({ tableId, target, items }) {
  ...
  await printTextWindows([{ type: "text", text }], normalizedTarget);
...
function buildPrintableReceiptItems(items) {
...
app.post("/api/print/receipt", async (req, res) => {
 ...
 try {
    ...
    const printableItems = buildPrintableReceiptItems(summary.items);

    const segments = buildReceiptText({
      tableId,
      items: printableItems.map((i) => ({
        name: i.name,
        qty: i.quantity,
        priceEx: i.price,
      })),
    ...
    await printTextWindows(segments, "receipt");
