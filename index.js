...
function buildOrderSlipText({ tableId, target, items }) {
  ...
  for (const item of items) {
    const name = String(item.name ?? "").padEnd(14, " ");
    const qty = `×${Number(item.quantity ?? 0)}`.padStart(4, " ");
    lines.push(`${name}${qty}`);
  }
...
async function printOrderSlip({ tableId, target, items }) {
  ...
  const printableItems = sourceItems.filter((item) => {
    ...
    return itemTarget === normalizedTarget && Number(item.quantity ?? 0) > 0;
  });
  ...
  await printTextWindows(text, normalizedTarget);
...
app.post("/api/print/order", async (req, res) => {
  console.log("PRINT ORDER API CALLED", req.body);
...
const printGroup = it.printGroup ?? "kitchen";

    const normalized = {
      id: itemId,
      printTarget,        // drink / food
      printGroup,         // kitchen / register
      printed: { drink: false, food: false, receipt: false },
      orderId,
      tableId,
      ...
      quantity,
      printTarget,  
      printGroup: it.printGroup ?? printTarget,               // 'kitchen' / 'register'
    ...
    if (normalized.productId) {
      ...
      store.addTableItemSnapshot(tableId, {
        ...
        printGroup: normalized.printGroup,
        price,
        qty: quantity,
        addedBy: normalized.orderedBy,
      });
