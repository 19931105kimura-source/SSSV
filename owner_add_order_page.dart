...
class _OwnerAddOrderPageState extends State<OwnerAddOrderPage> {
  ...
  String _setSection = 'normal'; // normal / agency / extension
...
// ============================================================
 Widget _setItems(BuildContext context) {
  ...
  final sets = context.watch<SetData>().sets;
...
,
  ...
  Widget _sectionTab(String text, String key) {
    final selected = _setSection == key;
    return Padding(
      padding: const EdgeInsets.only(right: 8),
      child: ChoiceChip(
        label: Text(text),
        selected: selected,
        onSelected: (_) => setState(() => _setSection = key),
      ),
    );
  }
...
,
  ...
  void _add(
    ...
    context.read<OrderState>().addManual(
  table: widget.table,
  category: category,
  brand: brand,
  label: label,
  price: price,
  section: category == 'セット' ? '案内所' : '',
...
class _OrderSlipPanelState extends State<_OrderSlipPanel> {
  ...
  Future<void> _rtUpdateQty({
  required String table,
  required OrderLine line,
  required int qty,
}) async {
  if (line.lineId == null) return; // ★ 追加
  if (qty <= 0) {
    await _rtRemoveLine(table: table, line: line);
    return;
  }

  try {
    final uri = Uri.parse(
      '$_serverBaseUrl/api/rt/tables/$table/items/${line.lineId}', // ★ 修正
    );
...
Future<void> _rtRemoveLine({
  ...
  try {
    final uri = Uri.parse(
      '$_serverBaseUrl/api/rt/tables/$table/items/${line.lineId}',
    );
 ...
 int _orderPriority(OrderLine l) {
  ...
  Widget build(BuildContext context) {
    ...
    final order = orderState.orderForDisplay(widget.table);
...
lines.sort((a, b) {
              ...
              child: lines.isEmpty
                  ...
                  : ListView.separated(
                      ...
                      itemBuilder: (context, i) {
                        final l = lines[i];
                        final title = l.label.trim().isEmpty
                            ? l.brand
                            : '${l.brand} ${l.label}';
  ...
  onPressed: () async {
    ...
    if (isRT) {
      ...
      // ★ 従来どおり
      if (newQty <= 0) {
        await context.read<OrderState>().removeLine(order!.id, l);
      } else {
        await context.read<OrderState>().updateQty(order!.id, l, newQty);
      }
        }
  },
),
